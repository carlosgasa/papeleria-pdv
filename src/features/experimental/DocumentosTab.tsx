...
import { ajustarSinRecortar, cargarImagen } from './imageUtils';
import { imprimirPaginas } from './imprimir';

interface EntradaDocumento {
  id: string;
  archivo: File;
...
const COPIAS_INICIALES = 1;

export function DocumentosTab() {
  const [entradas, setEntradas] = useState<EntradaDocumento[]>([]);
  const [plantillaId, setPlantillaId] = useState(PLANTILLAS_DOCUMENTO[0].id);
  const [anchoPersonalizado, setAnchoPersonalizado] = useState(10);
  const [altoPersonalizado, setAltoPersonalizado] = useState(15);
  ...
  async function exportarPdf() {
    ...
    try {
      ...
      const anchoMaxPx = Math.round((plantilla.anchoCm / CM_A_PULGADA) * DPI_EXPORTACION);
      const altoMaxPx = Math.round((plantilla.altoCm / CM_A_PULGADA) * DPI_EXPORTACION);
      const pxACm = CM_A_PULGADA / DPI_EXPORTACION;
      ...
      paginas.forEach((pagina, indicePagina) => {
        ...
        pagina.forEach((entrada, indiceSlot) => {
          ...
          const xCelda = margenCm + col * (plantilla.anchoCm + espacioCm);
          const yCelda = margenCm + fila * (plantilla.altoCm + espacioCm);
          const { dataUrl, anchoPx, altoPx } = ajustarSinRecortar(entrada.img, anchoMaxPx, altoMaxPx);
          const anchoCm = anchoPx * pxACm;
          const altoCm = altoPx * pxACm;
          const x = xCelda + (plantilla.anchoCm - anchoCm) / 2;
          const y = yCelda + (plantilla.altoCm - altoCm) / 2;
          pdf.addImage(dataUrl, 'JPEG', x, y, anchoCm, altoCm);
        });
      });
    ...
    <div className="space-y-4">
      <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
        <p className="text-xs text-gray-500 dark:text-gray-400">
          Sube la foto de un documento (acta, INE, comprobante) y acomoda varias copias reducidas o ampliadas en
          una sola hoja para imprimir. El documento nunca se recorta, solo se escala completo.
        </p>
        ...
        <label className="mt-3 block cursor-pointer rounded-lg border border-dashed border-gray-300 px-3 py-4 text-center text-sm text-gray-500 hover:border-brand-400 dark:border-gray-700 dark:text-gray-400">
          Agregar documento(s) (foto o escaneo)
          <input
            type="file"
            accept="image/*"
            multiple
          ...
          <div className="mt-3 space-y-2">
            {entradas.map((entrada) => (
              <div key={entrada.id} className="flex items-center gap-2 rounded-lg border border-gray-200 p-2 dark:border-gray-800">
                <img src={entrada.previewUrl} alt="" className="h-10 w-10 rounded-md bg-white object-contain" />
      ...
      <div className="grid grid-cols-2 gap-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900 md:grid-cols-4">
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-700 dark:text-gray-300">Tamaño destino</label>
          <select
            value={plantillaId}
      ...
      <p className="text-xs text-gray-500 dark:text-gray-400">
        {slotsPorHoja > 0
          ? `${columnas}×${filas} = ${slotsPorHoja} copias por hoja · ${paginas.length} hoja(s) en total`
            ...
            >
              ...
              >
                {pagina.map((entrada, indiceSlot) => (
                  <div key={`${entrada.id}-${indiceSlot}`} className="flex h-full w-full items-center justify-center border border-gray-200">
                    <img src={entrada.previewUrl} alt="" className="h-full w-full object-contain" />
                  </div>
                ))}
              </div>
            </div>
